...
module tt_um_andreaan1414_quickdraw (
    ...
    // ----------------------------------------------------------------
    // Quarter-second pulse (50 MHz clock -> pulse every 12,500,000 ticks)
    // ----------------------------------------------------------------
    reg [23:0] qsec_cnt;
    reg        qsec;
    always @(posedge clk) begin
    ...
    // ----------------------------------------------------------------
    // Score counters (3-bit, cap at 3 = first to 3 wins)
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // 7-segment display - show round result on the single on-board digit
    //
    //  A (winner A)  = segments for letter 'A'
    //  b (winner B)  = segments for letter 'b'
    //  C (both win)  = segments for letter 'C'
    //  L (both lose) = segments for letter 'L'
    //  blank (idle / green on) = all segments off
    //  Segments: {g, f, e, d, c, b, a}  (standard mapping)
    // ----------------------------------------------------------------
    reg [6:0] seg;
    always @(*) begin
